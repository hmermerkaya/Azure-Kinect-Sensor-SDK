...
private:
	...
	cv::Mat color1, ir1, color2, ir2, color3, ir3;
...
*/
     ...
     if(useExact)
                    {
                      syncsExact.push_back(boost::make_shared<message_filters::Synchronizer<ExactSyncPolicy>>(ExactSyncPolicy(queueSize),
                                  *subIRImages[0], *subColorImages[0]
                      ));
                      syncsExact.back()->registerCallback(boost::bind(&Receiver::callback, this, _1, _2));
                    }
                    else
                    {
                      syncsApproximate.push_back(boost::make_shared<message_filters::Synchronizer<ApproximateSyncPolicy>>(ApproximateSyncPolicy(queueSize),
                      ...
                      syncsApproximate.back()->registerCallback(boost::bind(&Receiver::callback, this, _1, _2));
                    }
...
private:
  ...
  void callback(const sensor_msgs::Image::ConstPtr imageIR1, const sensor_msgs::Image::ConstPtr imageColor1 )
  {
    cv::Mat ir1, color1;
    ...
    //std::cout<<"timestamps:" <<time_stamp1<<std::endl;
    this->color1 = color1;
    this->ir1 = ir1;
    updateImage = true;
    lock.unlock();
