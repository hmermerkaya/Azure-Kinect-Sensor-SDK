...
private:
	std::mutex lock;
	std::vector<std::tuple<std::string, std::string, std::string, std::string, std::string, std::string, std::string >> topicsDepthAndColorCamInfos_;
	...
	//std::map<std::string, std::pair<cv::Mat,cv::Mat>> mapSerialColorLookups;
    	std::map<std::string, cv::Mat> mapSerialDepthCamDistorInfo;

	std::map<std::string, cv::Mat> mapSerialDepthXYTables;
    	std::map<std::string, cv::Mat> mapSerialDepthCamInfo;
...
public:
  ...
  void start()
  {
	...
	std::transform (subCamerasInfoDepth.begin(), subCamerasInfoDepth.end(), std::inserter(mapSerialDepthCamInfo,mapSerialDepthCamInfo.end()),
        ...
        std::transform (subCamerasInfoDepth.begin(), subCamerasInfoDepth.end(), std::inserter(mapSerialDepthCamDistorInfo,mapSerialDepthCamDistorInfo.end()),

                                                        [this](sensor_msgs::CameraInfo::ConstPtr camInfo )  {


                                                        cv::Mat tmpCamInfo;
                                                        readCamDistortionInfo(camInfo, tmpCamInfo);
...
static void createCloud(const cv::Mat &depth, const cv::Mat & xy_table, pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud)
  ...
  void readCamDistortionInfo(const sensor_msgs::CameraInfo::ConstPtr cameraInfo, cv::Mat &distortionMatrix) const
  {
    distortionMatrix  = cv::Mat::zeros(1, 8, CV_64F);
    double *itC = distortionMatrix.ptr<double>(0, 0);
    for(size_t i = 0; i < 8; ++i, ++itC)
    {
      *itC = cameraInfo->D[i];
    }
 ...
 void createTopicDepthandImagesCamInfosTuple(const std::vector<std::string>  &serials) {
  	...
  	// for(auto & s: serials ) {
  	//   topicsDepthAndColorCamInfos_.push_back(std::make_tuple("/k4A_" + s + "/hd/image_depth", "/k4A_" + s + "/hd/image_ir",
...
//        imshow("",color1);
    ...
    //	cv::imwrite(frame_id2 + "_" + std::to_string(file_cnt)+ "_1.png", color2, params);
        cv::imwrite(frame_id1 +  "_ir.png", color1, params);
        cv::imwrite(frame_id2 +  "_ir.png", color2, params);
        ...
        for (auto &s:serials_) {
          ...
          storage <<"caminfo"<<mapSerialDepthCamInfo[s];
          storage.release();
          cv::FileStorage storage1(s+"_depth_calib.xml", cv::FileStorage::WRITE);
          storage1 <<"caminfo"<< mapSerialDepthCamInfo[s];
          storage1.release();
	   cv::FileStorage storage2(s+"_depth_distortion.xml", cv::FileStorage::WRITE);
	   storage2 <<"caminfo"<<mapSerialDepthCamDistorInfo[s];
	   storage2.release();
    ...
    //	  cv::imwrite(frame_id2 + "_" + std::to_string(file_cnt) + "_1_depth.png", depth2, params);
    	  cv::imwrite(frame_id1 +  "_depth.png", depth1, params);
        cv::imwrite(frame_id2 +  "_depth.png", depth2, params);
