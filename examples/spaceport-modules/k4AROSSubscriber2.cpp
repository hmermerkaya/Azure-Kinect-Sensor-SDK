...
class Producer
{
  private:
		...
		typedef std::vector<std::tuple<std::string, std::string, std::string, std::string, std::string, std::string>> stringTupleVector;
...
//				stamps[3]=imageColor4->header.stamp.toNSec();
    ...
    void createTopicDepthandImagesCamInfosTuple(const std::vector<std::string>  &serials) {
		...
		for(auto & s: serials ) {
  			  ...
  			  "/k4A_" + s + "/hd/depth_camera_info", "/k4A_" + s + "/hd/ir_camera_info", "/k4A_" + s + "/hd/color_camera_info"));
...
//	receiver.stop();
	...
	Producer producer (buf, serials , useExact, useCompressed);
