...
//        imshow("",color1);
	...
	cv::imwrite(frame_id1 + "_color.png", color1, params);
	cv::imwrite(frame_id2 + "_color.png", color2, params);
...
*/
        for (auto &s:serials_) {
          cv::FileStorage storage(s+"_color_calib.xml", cv::FileStorage::WRITE);
          storage <<"caminfo"<<mapSerialColorCamInfo[s];
          storage.release();
          cv::FileStorage storage1(s+"_depth_calib.xml", cv::FileStorage::WRITE);
          storage1 <<"caminfo"<< mapSerialIRCamInfo[s];
          storage1.release();

          create_xy_table(mapSerialColorXYTables[s], s+"_tablexy.data");
    ...
    //	  cv::imwrite(frame_id2 + "_" + std::to_string(file_cnt) + "_1_depth.png", depth2, params);
	  ...
	  cv::imwrite(frame_id1 + "_ir.png", ir1, params);
          cv::imwrite(frame_id2 + "_ir.png", ir2, params);
