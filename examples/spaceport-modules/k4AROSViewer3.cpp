...
private:
	...
	const bool useExact, useCompressed;
  	...
  	bool ircolorOk=false, depthOk=false;
	...
	std::string frame_id1, frame_id2, frame_id3, time_stamp1, time_stamp2;
	std::string frame_depth_id1, frame_depth_id2, frame_depth_id3, time_depth_stamp1, time_depth_stamp2;
	cv::Mat color1, ir1, color2, ir2, color3, ir3, depth1, depth2, depth3;
...
//	typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image, sensor_msgs::Image, sensor_msgs::Image, sensor_msgs::Image, sensor_msgs::CameraInfo, sensor_msgs::CameraInfo, sensor_msgs::CameraInfo, sensor_msgs::CameraInfo> ApproximateSyncPolicy;
	...
	typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image, sensor_msgs::Image, sensor_msgs::Image> ApproximateSyncPolicyDepth;
	ros::NodeHandle nh;
	...
	boost::shared_ptr<message_filters::Synchronizer<ApproximateSyncPolicyDepth> > syncsApproximateDepth;
...
//                    {
//                      syncsExact.push_back(boost::make_shared<message_filters::Synchronizer<ExactSyncPolicy>>(ExactSyncPolicy(queueSize),
//                                  *subIRImages[0], *subIRImages[1], *subIRImages[2], *subColorImages[0], *subColorImages[1], *subColorImages[2], *subDepthImages[0], *subDepthImages[1], *subDepthImages[2]
//                      ));
//                      syncsExact.back()->registerCallback(boost::bind(&Receiver::callback, this, _1, _2, _3, _4, _5, _6, _7, _8, _9));
//                    }
//                    else
//                    {
//                      syncsApproximate.push_back(boost::make_shared<message_filters::Synchronizer<ApproximateSyncPolicy>>(ApproximateSyncPolicy(queueSize),
//                                  *subIRImages[0], *subIRImages[1], *subIRImages[2], *subColorImages[0], *subColorImages[1], *subColorImages[2], *subDepthImages[0], *subDepthImages[1], *subDepthImages[2]
//                      ));
//                      syncsApproximate.back()->registerCallback(boost::bind(&Receiver::callback, this, _1, _2, _3, _4, _5, _6, _7, _8, _9));
//                    }



	syncsApproximate = boost::make_shared<message_filters::Synchronizer<ApproximateSyncPolicy>>(ApproximateSyncPolicy(queueSize),
                                  *subIRImages[0], *subIRImages[1], *subIRImages[2], *subColorImages[0], *subColorImages[1], *subColorImages[2]
                      );

        syncsApproximateDepth=boost::make_shared<message_filters::Synchronizer<ApproximateSyncPolicyDepth>>(ApproximateSyncPolicyDepth(queueSize),*subDepthImages[0], *subDepthImages[1], *subDepthImages[2]
                      );

	syncsApproximate->registerCallback(boost::bind(&Receiver::callback, this, _1, _2, _3, _4, _5, _6));
	syncsApproximateDepth->registerCallback(boost::bind(&Receiver::callback1, this, _1, _2, _3));
...
private:
  ...
  void callback(const sensor_msgs::Image::ConstPtr imageIR1, const sensor_msgs::Image::ConstPtr imageIR2 ,const sensor_msgs::Image::ConstPtr imageIR3 ,
		  const sensor_msgs::Image::ConstPtr imageColor1, const sensor_msgs::Image::ConstPtr imageColor2, const sensor_msgs::Image::ConstPtr imageColor3 
		  
		  )
  {
    cv::Mat ir1, ir2 ,ir3, color1, color2, color3;
...
void callback1(const sensor_msgs::Image::ConstPtr imageDepth1, const sensor_msgs::Image::ConstPtr imageDepth2,const sensor_msgs::Image::ConstPtr imageDepth3
                  ...
                  )
  {
    ...
    readImage(imageDepth3, depth3);



    //lock.lock();
    frame_depth_id1=stripStrTillEnd(imageDepth1->header.frame_id, "_");//std::to_string(imageDepth1->header.stamp.toNSec());;
    frame_depth_id2=stripStrTillEnd(imageDepth2->header.frame_id, "_");
    frame_depth_id3=stripStrTillEnd(imageDepth3->header.frame_id, "_");

    time_depth_stamp1 = stripStrTillEnd(std::to_string(imageDepth1->header.stamp.toNSec()/1000000), "_");
    time_depth_stamp2 = stripStrTillEnd(std::to_string(imageDepth2->header.stamp.toNSec()/1000000), "_");
   // std::cout<<"timestamps:" <<time_stamp1<<" "<<time_stamp2<<std::endl;
...
static void createCloud(const cv::Mat &depth, const cv::Mat & xy_table, pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud)
 ...
 void imageViewer()
  {
    cv::Mat color1, ir1, color2, ir2, color3, ir3, depth1, depth2, depth3;
    cv::namedWindow("IR Image 1", cv::WINDOW_NORMAL);
    cv::moveWindow("IR Image 1", 250,0);
//    cv::resizeWindow("IR Image 1", 900, 600);
    cv::namedWindow("IR Image 2");
    cv::moveWindow("IR Image 2", 500,0);

     cv::namedWindow("IR Image 3");
    cv::moveWindow("IR Image 3", 800,0);
...
//
	    ...
	    boost::mutex::scoped_lock lk(mutex);
            ...
            data_ready_cond_.wait(lk, [this] { return ( ircolorOk && depthOk  ); });
         //   std::cout<<"Inside imageviewer 1\n";
            ircolorOk=false; depthOk=false;
            ...
            cv::imshow("IR Image 1", ir1);
   //           cv::moveWindow("IR Image 1", 250,0);
            cv::imshow("IR Image 2", ir2);
 //             cv::moveWindow("Color Image 2", 800,0);
            cv::imshow("IR Image 3", ir3);
	...
	cv::imwrite(frame_id1 + "-" + tmstp + "-rgb.png", color1, params);
        ...
        cv::imwrite(frame_id3 + "-" + tmstp + "-rgb.png", color3, params);
 ...
 //         cv::FileStorage storage1(s+"_depth_calib.xml", cv::FileStorage::WRITE);
   //       storage1 <<"caminfo"<< mapSerialColorCamInfo[s];
    ...
    //	  cv::imwrite(frame_id2 + "_" + std::to_string(file_cnt) + "_1_depth.png", depth2, params);
	  ...
	  cv::imwrite(frame_depth_id1 + "-" + tmstp + "-depth.png", depth1, params);
          cv::imwrite(frame_depth_id2 + "-" + tmstp + "-depth.png", depth2, params);
          cv::imwrite(frame_depth_id3 + "-" + tmstp + "-depth.png", depth3, params);
...
int main(int argc, char **argv)
{
	...
	ros::init(argc, argv, "azure kinect viewer");
	...
	try {
		...
		desc.add_options()
		 ("serials,s", po::value<std::vector<std::string> >()->multitoken(), "azure_kinect_serials")
